...
def test_artifact_complete_e2e_reloads_on_demand_and_cleans_mount() -> None:
    ...
    agent = AgentBuilder().provider(provider).build(session_id="session_artifact_e2e")
    ...
    _assert_mount_released(agent, record.id)

    second_result = asyncio.run(agent.run("请重新查看之前的图纸"))
...
def _upload(agent: Agent) -> ArtifactRecord:
    return agent.artifacts.upload(
        data=_ARTIFACT_DATA,
        filename=_FILENAME,
        media_type=_MEDIA_TYPE,
    )
...
def _assert_mount_released(agent: Agent, handle: str) -> None:
    assert agent.artifacts.active_mounts() == ()
    assert agent.artifacts.read(handle) == _ARTIFACT_DATA
