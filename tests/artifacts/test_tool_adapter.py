...
def test_adapter_handlers_are_bound_to_one_artifact_runtime() -> None:
    target = runtime()
    record = target.upload(
    ...
    listed = tools["list_attachments"].handler({})
    ...
    loaded = tools["load_attachment"].handler({"handle": record.id})
    assert loaded == (
        f"附件已挂载：{record.id}。"
