...
@async_test
async def test_planner_runtime_dispatches_ready_steps_with_limit() -> None:
    ...
    report = await runtime.dispatch_ready_steps("plan_1", limit=1)
...
@async_test
async def test_planner_runtime_dispatch_ready_steps_reports_template_skips() -> None:
    coordinator = FakeCoordinator()
    runtime = PlannerRuntime(
        store=InMemoryPlanStore(),
        dispatcher=coordinator,
    ...
    report = await runtime.dispatch_ready_steps(
        "plan_1",
...
@async_test
async def test_planner_runtime_dispatch_ready_steps_reports_coordinator_failure() -> (
    None
):
...
@async_test
async def test_planner_runtime_dispatch_failure_does_not_overwrite_concurrent_step_completion() -> (
    None
):
