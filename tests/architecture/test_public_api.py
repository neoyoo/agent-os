...
def _root_facade_import_contract_violations(
    source: str,
    ...
    for node in ast.walk(tree):
        if isinstance(node, (ast.Import, ast.ImportFrom)) and not isinstance(
            ...
            violations.append(f"non-top-level-import:{node.lineno}")
        ...
        if isinstance(node, ast.ImportFrom):
            ...
            for alias in node.names:
                if alias.name == "*":
                    violations.append(f"star-import:{node.module}:{node.lineno}")
                    continue
                imported_name = alias.asname or alias.name
                if alias.name != imported_name:
                    violations.append(
                        f"renamed-import:{alias.name}:{imported_name}:{node.lineno}",
                    )
                    continue
                if imported_name not in public_names:
                    violations.append(
                        f"undeclared-import:{imported_name}:{node.lineno}",
                    )
        elif isinstance(node, ast.Import):
            for alias in node.names:
                imported_name = alias.asname or alias.name.split(".", 1)[0]
                if alias.asname is not None and alias.name != imported_name:
                    violations.append(
                        f"renamed-import:{alias.name}:{imported_name}:{node.lineno}",
                    )
                    continue
            ...
            if (
                ...
                violations.append(f"dynamic-import:{node.lineno}")

    return tuple(violations)


@pytest.mark.parametrize(
    ("source", "public_names", "expected_violation"),
    [
        (
        ...
        (
            "import agentos.runtime as RELEASE_EVIDENCE_REQUIRED_GATES\n",
            {"RELEASE_EVIDENCE_REQUIRED_GATES"},
            "renamed-import:agentos.runtime:RELEASE_EVIDENCE_REQUIRED_GATES:1",
        ),
        (
        ...
        (
            "from agentos.runtime import HiddenRuntime\n",
            set(),
            "undeclared-import:HiddenRuntime:1",
        ),
    ],
)
def test_root_facade_import_contract_rejects_forbidden_mutations(
    source: str,
    public_names: set[str],
    expected_violation: str,
) -> None:
    violations = _root_facade_import_contract_violations(source, public_names)
