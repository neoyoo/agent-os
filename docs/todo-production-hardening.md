...
SDK 核心引擎已达 production-grade 架构质量（Protocol 边界、CAS 状态机、零硬依赖、482 测试全过）。
...
**要求**：
- [ ] 在 `QueryLoop._consume_provider_stream` 层加入可配置 retry（指数 backoff + jitter）
- [ ] 支持 `max_retries`、`retry_on`（异常类型白名单）、`backoff_base` 配置
- [ ] 连续失败 N 次进入 circuit-open 状态，快速 fail 避免雪崩
- [ ] retry 期间发出 `ProviderRetryEvent`，可观测
...
**要求**：
- [ ] `SpawnExecutor.shutdown(timeout_seconds)` 等待运行中任务完成或 timeout 后 cancel
- [ ] `ExpertAgentRunner.stop(timeout_seconds)` 等待当前 task 执行完后退出
- [ ] `AsgiAgentApp` 支持 ASGI lifespan protocol（startup / shutdown 事件）
- [ ] shutdown 时 TaskTable 中 running 任务标记为可重新 claim（释放 lease）
...
**现状**：ASGI app 无健康检查路由。
**要求**：
- [ ] `GET /health` 返回 `{"status": "ok"}` + 200
- [ ] `GET /ready` 检查 provider 可达（可选）、DB 连接（可选）后返回状态
- [ ] 支持注入自定义 health check callable
...
**要求**：
- [ ] 引入 `structlog` 或标准 `logging` + JSON formatter
- [ ] QueryLoop 关键节点写 structured log（turn_start, provider_call, tool_exec, turn_end）
- [ ] log 中携带 trace_id / session_id / turn_id
- [ ] 默认不输出，通过 `ObservabilityConfig.logging_enabled` 开启
...
**要求**：
- [ ] 基于 session_id 的滑动窗口限流（默认 60 req/min）
- [ ] 超限返回 429 + `Retry-After` header
- [ ] 可注入自定义 `RateLimiter` Protocol
...
**要求**：
- [ ] 项目 README：一句话定位 + 安装 + 最小示例（5 行代码跑起来）
- [ ] `docs/quickstart.md`：从零构建一个 tool-calling agent（10 分钟）
- [ ] `docs/architecture.md`：模块依赖图 + 数据流图（可用 mermaid）
...
**要求**：
- [ ] `examples/streaming_agent.py` — SSE streaming 部署
- [ ] `examples/multi_agent_dispatch.py` — expert dispatch 完整示例
- [ ] `examples/mcp_agent.py` — 连接 MCP server
- [ ] `examples/persistent_agent.py` — session 持久化 + 恢复
- [ ] 每个 example 可独立运行，有 `if __name__ == "__main__"` 入口
...
**现状**：无 CLI 工具。
**要求**：
- [ ] `agent-os init` 生成项目骨架（pyproject.toml + agent 配置）
- [ ] `agent-os run` 启动 ASGI server（内嵌 uvicorn）
- [ ] `agent-os migrate` 执行 Postgres migrations
...
**要求**：
- [ ] `Provider` Protocol 增加可选 `timeout_seconds` 参数
- [ ] AnthropicProvider / OpenAIProvider / OpenAICompatibleProvider 支持连接/读取超时
- [ ] 超时产生 `ProviderTimeoutError`，可触发 retry
...
**要求**：
- [ ] `ChannelTurnRequest` 加 max_message_length 校验（防止 prompt injection payload 过大）
- [ ] tool arguments 经过 JSON schema 校验后再传给 handler
- [ ] 敏感字段（api_key、connection string）不出现在 error message 和 log 中
...
**要求**：
- [ ] Postgres adapter 支持 connection pool（psycopg_pool）
- [ ] Redis adapter 支持 ConnectionPool + auto-reconnect
- [ ] 连接不可用时抛明确 `BackendUnavailableError`，上层可 retry
...
**现状**：无 CI 配置。
**要求**：
- [ ] `.github/workflows/ci.yml`：pytest + compileall + ruff lint
- [ ] `ruff.toml` 或 `pyproject.toml [tool.ruff]` 配置
...
**要求**：
- [ ] 后台 task 周期性扫描 `agentos_multi_agent_task_outbox` 中 `delivered_at IS NULL` 的行
- [ ] 对每条未投递记录重新 send result_ready envelope
- [ ] 投递成功后标记 `delivered_at`
- [ ] 可配置扫描间隔和批次大小
...
**要求**：
- [ ] 定期调用 `XPENDING` 检查 idle 超过阈值的 message
- [ ] 用 `XCLAIM` 重新分配给当前 consumer
- [ ] 超过最大重试次数的 message 转入 dead-letter stream
- [ ] 可配置 idle_threshold / max_retries
...
**要求**：
- [ ] 新增 `RedisContinuationTrigger`，通过 Redis Pub/Sub 跨节点通知 parent agent
- [ ] fallback 到 polling TaskStore（给不依赖 Redis 的部署）
- [ ] 与 ExpertAgentRunner 集成测试
...
**要求**：
- [ ] `tests/integration/` 目录，pytest mark `@pytest.mark.integration`
- [ ] Docker Compose 配置（postgres + redis）
- [ ] CI 中可选运行 integration tests
- [ ] 覆盖：concurrent claim、cancel race、outbox delivery、stream retry
