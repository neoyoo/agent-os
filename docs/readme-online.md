...
| `load_image` | 将已上传图片附件加载到下一次 provider request 供模型重新查看 |
...
图片附件走独立的 `AttachmentRuntime`。首轮上传的图片会作为 user message 的 `ImagePart` 投影给 provider；后续如果模型需要重新查看图片，必须调用 `load_image(handle="att:...")`，由 runtime 将该图片一次性加载到下一次 provider request。当前附件 runtime 只投影 image。
