...
- 不做自动 turn 入口 notification 注入；第一版结果只通过协调工具以 tool result 暴露给主 agent。
...
### 方案 A：Async 执行消息通道 + QueryLoop 自动 notification 注入
...
- 自动注入非 tool result 会绕过 “主 agent 只通过 tool result 吸收 subagent 发现” 的架构规则。
- `Bus` 在项目命名规则中只用于观察型 pub/sub，执行消息通道不能叫 bus。

### 方案 B：同步 AgentInbox + tool result 收取结果（采用）
...
- 主 agent 用 `check_agent_tasks` 主动读取结果，结果作为普通 tool result 进入 `MessageRuntime`。

这个方案与现有 SDK 边界最匹配，后续也可以把 `AgentInbox` 替换为远程 channel adapter，而不改变 LLM 看到的协调工具。
...
| 16 | FakeProvider + Spawn 模式端到端测试通过 | `tests/multi/test_coordinator_spawn.py` |
| 17 | FakeProvider + Discover & Dispatch 模式端到端测试通过 | `tests/multi/test_coordinator_dispatch.py` |
| 18 | 默认 prompt 不出现 `agent_id`、`task_id`、`envelope_id` 等 runtime metadata，除非作为 tool result | renderer golden / integration tests |
...
- MoA 纯推理合成。
- 自动 notification projection 或 QueryLoop turn 入口注入。
