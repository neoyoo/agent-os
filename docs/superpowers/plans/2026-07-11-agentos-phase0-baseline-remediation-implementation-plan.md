...
- Create: `tests/test_release_evidence_local.py`
...
只保留领域 validator 以及版本、template、artifact 静态契约，且必须低于 500 行；
`tests/test_release_evidence_local.py` 独立拥有 local candidate env gate/helper
和全部 `test_local_release_evidence_*` 契约，且必须低于 500 行；
`tests/test_release_evidence_cli.py` 独立拥有 validator CLI 与 generator
CLI/subprocess 契约，且必须低于 500 行；`tests/_release_evidence_fixtures.py`
只提供无副作用的纯 manifest builder。三个测试模块不得相互 import，只能显式
依赖共享纯 helper。拆分后测试名称、数量和语义不得丢失。
