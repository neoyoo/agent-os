...
A2、A3、A4 都会收窄 `src/agentos/channels/a2a_operations.py`，并更新 `src/agentos/transports/a2a/__init__.py`、`src/agentos/channels/__init__.py` 或 Public API inventory。上述共享 facade 和 integration 文件统一由 **A2A Facade Integration Owner** 串行修改；并行 worktree 只能修改各任务独占的实现模块和目标测试，不能各自提交共享 facade 变更。A2、A3、A4 按顺序集成并在每次 facade 变更后运行 compatibility tests，worktree 隔离不能替代这一单 Owner 门禁。
...
| Phase 6 wave 4 | A4、A5 与 D4 可并行；如果 H2/D2 因独立进度仍在执行，A4 也可与它们并行 | A4 不得与 A2/A3 重叠修改；A2A Facade Integration Owner 完成 A4 收口后，各 workstream 再执行 inventory drift 和兼容 facade 门禁 |
| Phase 6 integration | 公共 facade 不允许多 Owner 并行修改 | A2A 内部严格按 A2 -> A3 -> A4 串行集成；跨 workstream 按 A2A -> HTTP -> Deployment 顺序完成最终集成，每次集成后运行第 3 节全量契约；最后才允许删除已批准的旧 facade |
