...
A2、A3、A4、A5 都会收窄 `src/agentos/channels/a2a_operations.py` 或 `src/agentos/channels/a2a_conformance.py`，并更新 `src/agentos/transports/a2a/__init__.py`、`src/agentos/channels/__init__.py` 或 Public API inventory。上述共享 facade 和 integration 文件统一由 **A2A Facade Integration Owner** 串行修改；并行 worktree 只能修改各任务独占的实现模块和目标测试，不能各自提交共享 facade 变更。A2、A3、A4、A5 按顺序集成并在每次 facade 变更后运行 compatibility tests，worktree 隔离不能替代这一单 Owner 门禁。
...
| A2 | `*_to_dict`、`*_from_dict`、SSE event parsing、保留字段恢复及 JSON object 校验 | `src/agentos/transports/a2a/serialization.py`；`tests/transports/a2a/test_serialization.py` | **必须在 A1 后串行**，只依赖 `wire_types.py` 和稳定领域枚举；完成实现测试后，由 A2A Facade Integration Owner 串行集成兼容导出，随后 A3 才能迁移 mapping/router。 | Distributed Owner；共享 facade 由 A2A Facade Integration Owner | 旧序列化函数保留显式 re-export，直至新路径 golden/round-trip/legacy payload 测试绿色，且 `rg -n "a2a_(message|artifact|task|operation).*_(to|from)_dict|parse_a2a_sse_events" src tests` 不再发现计划外旧实现。 | `python -m pytest tests/transports/a2a/test_serialization.py tests/channels/test_a2a_operations.py -q`；`python -m compileall -q src/agentos/transports/a2a tests/transports/a2a` |
| A3 | official operation 映射、版本/extension negotiation、领域 `TaskRecord` 到 A2A task 的映射、server/client 单次 operation 路由 | `src/agentos/transports/a2a/mapping.py`；`src/agentos/transports/a2a/router.py`；`src/agentos/transports/a2a/client.py`；`tests/transports/a2a/test_mapping.py`；`tests/transports/a2a/test_router.py` | **必须在 A2 facade 集成和 compatibility tests 后串行**。先迁纯 mapping，再迁 router，最后迁 client；再由 A2A Facade Integration Owner 串行集成。Router 只调 Port/runner，不拥有 TaskStore、retry 或 push worker 生命周期。A4、A5 依赖本步稳定的新路径。 | Distributed Owner；共享 facade 由 A2A Facade Integration Owner | `A2AOperationServer`、`A2AOperationClient` 和历史 helper 在旧模块保留显式兼容导出；只有 inventory 已迁往新 canonical module、仓库调用方已切换、旧路径 compatibility test 明确批准弃用后，才允许删除旧实现。 | `python -m pytest tests/transports/a2a/test_mapping.py tests/transports/a2a/test_router.py tests/channels/test_a2a_operations.py tests/channels/test_a2a_server.py tests/channels/test_a2a_adapter.py -q`；`python -m ruff check src/agentos/transports/a2a src/agentos/channels/a2a_operations.py` |
| A4 | push notification config/policy、memory/PostgreSQL Store、delivery dispatcher、worker 和 daemon | `src/agentos/transports/a2a/push_types.py`；`src/agentos/adapters/a2a/push_memory.py`；`src/agentos/adapters/a2a/push_postgres.py`；`src/agentos/adapters/a2a/push_worker.py`；`tests/adapters/a2a/test_push_notifications.py` | **必须在 A3 facade 集成和 compatibility tests 后串行启动**。内部按 pure types -> memory contract -> PostgreSQL adapter -> worker/daemon 串行迁移，再由 A2A Facade Integration Owner 收口共享入口。A4 可与仍在执行且文件隔离的 H2 或 D2 并行，不得与 A2/A3 并行。不得让 `transports.a2a` 导入 PostgreSQL client。 | Distributed Owner；共享 facade 由 A2A Facade Integration Owner | 旧 push notification 名称在 `a2a_operations.py` 保持显式 re-export，直至 memory/PostgreSQL contract、daemon shutdown 和部署 profile 测试全部绿色；旧 Store/worker 实现必须通过 `rg -n "^class (PostgresA2A|InMemoryA2A|A2APushNotification)" src/agentos/channels/a2a_operations.py` 归零后才可删除 facade 内实现。 | `python -m pytest tests/adapters/a2a/test_push_notifications.py tests/channels/test_postgres_a2a_push_notifications.py tests/channels/test_a2a_push_notification_daemon.py tests/channels/test_a2a_egress_url_policy.py -q`；`python -m ruff check src/agentos/adapters/a2a src/agentos/transports/a2a/push_types.py` |
| A5 | conformance check/finding/report、external invocation plan/gates/runner、sample harness | `src/agentos/testing/a2a_conformance/types.py`；`src/agentos/testing/a2a_conformance/runner.py`；`src/agentos/testing/a2a_conformance/harness.py`；`tests/testing/a2a_conformance/test_runner.py`；`tests/testing/a2a_conformance/test_harness.py` | **必须在 A3 后启动**，先迁 report leaf types，再迁 external runner，最后迁 harness；独占实现模块和目标测试可与 A4 并行，但 `channels/a2a_conformance.py`、`channels/__init__.py` 和 inventory 只能在 A4 facade 集成后由 A2A Facade Integration Owner 串行收口。仅可依赖公开 transport 契约，不得导入 `channels.a2a_operations`；涉及 push 的检查等待 A4。 | Quality/Release Owner；共享 facade 由 A2A Facade Integration Owner | `agentos.channels.a2a_conformance` 保留薄 re-export，直到新测试路径覆盖全部报告/外部执行契约、`rg -n "channels\.a2a_conformance|channels\.a2a_operations" src/agentos/testing/a2a_conformance tests/testing/a2a_conformance` 无反向依赖，且 Public API inventory 明确 canonical module。 | `python -m pytest tests/testing/a2a_conformance tests/channels/test_a2a_conformance.py tests/architecture/test_public_api.py -q`；`python -m compileall -q src/agentos/testing/a2a_conformance tests/testing/a2a_conformance` |
...
| Phase 6 wave 2 | A2、H2、D2 可并行；三者文件 Owner 隔离 | A2 的实现和共享 facade 集成均通过后才能开始 A3；H2 通过后才能开始 H3；D2 通过后才能开始 D3 |
| Phase 6 wave 3 | A3、H3、D3 可并行；A3 的共享 facade 仍由单一 Owner 串行集成 | A3 compatibility tests 通过后才能开始 A4/A5；D3 完成后才能迁 D4 consumers |
| Phase 6 wave 4 | A4、A5 与 D4 的独占实现模块和目标测试可并行；如果 H2/D2 因独立进度仍在执行，A4 也可与它们并行 | A4/A5 并行 worker 禁止修改 legacy facade、`channels/__init__.py` 和 inventory；A2A Facade Integration Owner 必须先串行完成 A4 收口，再完成 A5 收口 |
| Phase 6 integration | 公共 facade、inventory 和 legacy facade 不允许多 Owner 并行修改 | A2A 内部严格按 A2 -> A3 -> A4 -> A5 串行集成；跨 workstream 按 A2A -> HTTP -> Deployment 顺序完成最终集成，每次集成后运行第 3 节全量契约；最后才允许删除已批准的旧 facade |
