...
当前 multi-agent coordination 已有协议边界和 adapter：`TaskStore` / `AgentMessageQueue` 是分布式任务与消息边界，`TaskTable` / `AgentInbox` 是 in-memory adapter，`PostgresTaskStore` / `RedisAgentMessageQueue` 是生产 adapter 骨架。但 live Postgres/Redis 集成测试、Redis retry/reclaim、outbox reconciler 和跨节点 continuation trigger 仍未完成。
...
源码来源：[`src/agentos/multi/coordinator.py`](../src/agentos/multi/coordinator.py)，[`src/agentos/multi/task_store.py`](../src/agentos/multi/task_store.py)，[`src/agentos/multi/message_queue.py`](../src/agentos/multi/message_queue.py)，[`src/agentos/multi/tasks.py`](../src/agentos/multi/tasks.py)，[`src/agentos/multi/inbox.py`](../src/agentos/multi/inbox.py)，[`src/agentos/multi/postgres_tasks.py`](../src/agentos/multi/postgres_tasks.py)，[`src/agentos/multi/redis_queue.py`](../src/agentos/multi/redis_queue.py)，[`src/agentos/multi/remote.py`](../src/agentos/multi/remote.py)，[`src/agentos/channels/a2a.py`](../src/agentos/channels/a2a.py)
...
| 缺口 | 当前事实 | 影响 |
|---|---|---|
| live Postgres/Redis integration | 当前只有 fake/contract tests | 尚未验证真实服务上的事务、consumer group pending/retry 和连接错误 |
| outbox reconciler | migration 和 outbox 写入已存在，但没有后台 reconciler | result-ready 通知失败后还不能自动补发 |
| Redis pending/retry | `RedisAgentMessageQueue` 支持 stream delivery/ack，但未实现 XPENDING/XCLAIM retry | worker crash 后 pending message 的重新领取仍未完成 |
| continuation trigger 跨节点唤醒 | 当前 `ContinuationTrigger` 仍是本地触发器 | parent agent 不在同一进程时还不能自动唤醒 |
...
- [ ] 增加 reconciler，周期性扫描 outbox 或未通知终态任务并补发 `result_ready`。
- [x] 新增 migration，包含 up/down。
- [ ] 添加并发 transition 测试，验证重复 claim、重复 terminal write、cancel/complete 竞态、late result 都是幂等或被拒绝。
...
- [ ] 支持 pending/ack/retry，避免 worker crash 后 envelope 永久丢失。
...
- [ ] 添加 Redis fake 或 contract tests，覆盖 send/collect/wait/backpressure/retry。
...
- [ ] `dispatch` 对 endpoint-backed agent 继续支持 A2A remote task；对 queue-backed worker 走 distributed message queue。
...
- [ ] continuation trigger 需要支持跨节点唤醒，不能只依赖本地 `AgentInbox` event。
...
- [ ] 保留现有 in-memory multi-agent 测试全部通过。
- [ ] 增加 distributed contract tests，同一套测试跑 in-memory、Postgres task store、Redis message queue fake。
- [ ] 增加 crash/retry 场景：worker claim 后崩溃、parent 未 collect、result late arrival、cancel 与 complete 竞态。
- [ ] 增加 A2A remote dispatch 与 distributed task store 的集成测试。
- [ ] 增加 trace context 传播测试，确保 remote task 的 trace headers 继续传递。
...
- [ ] 不在这一项里重写 `QueryLoop`。
- [ ] 不把 `EventBus` 变成消息队列；`EventBus` 仍然只做观察。
- [ ] 不把 runtime metadata 渲染进默认 prompt。
- [ ] 不默认引入 Nacos；当前项目已有 Postgres registry 和 A2A 基础，优先补齐 Postgres/Redis 边界。
