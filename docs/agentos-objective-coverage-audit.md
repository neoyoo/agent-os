...
`QueryLoop` and `AsyncQueryLoop`. The canonical files are
...
| Single-Node Async Web Agents | Direct | `AsyncQueryLoop`, `WebRuntimeProfile`, `AsgiAgentApp`, `AsyncAgentSessionProvider`, SSE/JSON turn endpoints | App-specific auth policy, workspace policy, and live backend verification remain deployment-owned. |
...
| Sync Versus Async Loop Guidance | Direct | `LocalRuntimeProfile(loop_mode="sync")`, `LocalRuntimeProfile(loop_mode="async")`, `AsyncQueryLoop`, `Agent.async_run`, production guidance in `.claude/skills/agent-os/modules/agent-forms.md` | Guidance must remain conservative: terminal/script defaults to sync; web hosts prefer async when native async providers/tools are available. |
