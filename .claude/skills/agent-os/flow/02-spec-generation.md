...
deployment:
  ...
  state_plane:
    ...
    worker_process_supervisor_boundary:
      ...
      evidence_fields: [status, pid, exit_code, started_at, stop_requested_at, stopped_at, error, worker_kind, command, env_keys]
...
multi_agent:
  ...
  team:
    ...
    worker_lifecycle:
      ...
      evidence_fields: [exit_code, started_at, stop_requested_at, stopped_at, env_keys]
      command_policy: argv-only
      shell_parsing: no shell parsing
      production_gap: not a Kubernetes, systemd, autoscaling, or secret-distribution layer
    state_plane:
