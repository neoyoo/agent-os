...
def run_plan_store_contract(
    factory: Callable[[], PlanStore],
    *,
    seed_claim: Callable[[PlanStore, PlanClaimRecord], None] | None = None,
) -> None:
    _assert_create_save_load_and_owner_filter(factory)
    _assert_compare_and_save_contract(factory)
    _assert_claim_guarded_save_contract(factory, seed_claim=seed_claim)


def _assert_create_save_load_and_owner_filter(
    factory: Callable[[], PlanStore],
) -> None:
    store = factory()
    ...
    check_equal(store.get_plan(original.plan_id), original, "get_plan must load plan")
    check_is(store.get_plan("missing_plan"), None, "missing get_plan must return None")
    check_equal(
        store.list_plans("leader"),
        [original],
        "list_plans must filter by owner",
    )
    check_equal(
        store.list_plans("other_leader"),
        [other],
    ...
    check_equal(store.get_plan(original.plan_id), updated, "save_plan must update plan")


def _assert_compare_and_save_contract(factory: Callable[[], PlanStore]) -> None:
    store = factory()
    ...
    check_is(
        save_if_unchanged(updated, expected_revision=record.revision),
        True,
    ...
    check_is(
        save_if_unchanged(stale_update, expected_revision=record.revision),
        False,
        "save_plan_if_unchanged must reject stale revision",
    )
    check_equal(
        store.get_plan(original.plan_id),
        updated,
...
def _assert_claim_guarded_save_contract(
    factory: Callable[[], PlanStore],
    *,
    seed_claim: Callable[[PlanStore, PlanClaimRecord], None] | None,
) -> None:
    store = factory()
...
def _assert_plan_not_found(action: Callable[[], object]) -> None:
    try:
        action()
