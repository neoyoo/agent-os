...
_COORDINATION_MODES = frozenset({"spawn", "dispatch"})
_ENVELOPE_TYPES = frozenset({"task_request", "task_result"})
...
def task_record_to_dict(record: TaskRecord) -> JsonDict:
    ...
    return {
        ...
        "result": (
            None if record.result is None else task_result_to_dict(record.result)
        ),
        "late_result": (
            None
...
def task_record_from_dict(data: JsonDict) -> TaskRecord:
    ...
    return TaskRecord(
        ...
        result=(
            ...
            else task_result_from_dict(data["result"])
        ),
        late_result=(
            None
...
def envelope_from_dict(data: JsonDict) -> AgentEnvelope:
    ...
    if envelope_type == "task_request":
        ...
        payload: TaskRequest | TaskResult = task_request_from_dict(data["payload"])
    else:
